(function () {
  if (!document.getElementById('grid2048')) return;
  ...
  function init2048() {
    ...
    function tileColor(v) {
      ...
      var p = Math.log2(v) / 11; // 2^11=2048
      p = clamp(p, 0, 1);
      var a = 0.18 + p * 0.28;
  ...
  function initSnake() {
    ...
    function draw() {
      ...
      for (var i = 1; i < cells; i++) {
        ...
        ctx.lineTo(i * gridSize, cells * gridSize);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(0, i * gridSize);
        ctx.lineTo(cells * gridSize, i * gridSize);
        ctx.stroke();
      }
    ...
    window.addEventListener('keydown', function (e) {
      if (!document.getElementById('gsnake')) return;
      var k = e.key;
